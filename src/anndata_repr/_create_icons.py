...
def get_display(adata):
    ...
    css_content = (pathlib.Path(__file__).parent / "static/style.css").read_text(
        encoding="utf-8"
    )
...
def get_svg(adata):
    ...
    # JAVSCRIPT
    js_content = (pathlib.Path(__file__).parent / "hover_icons.js").read_text(
        encoding="utf-8"
    )
    ...
    x = get_layers(
        ...
        size_cols_x,
        size_rows_x,
        f"({adata.X.shape[0]}, {adata.X.shape[1]}, {n_layers_x})",
    )
