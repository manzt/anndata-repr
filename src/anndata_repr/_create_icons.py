...
def get_display(adata: anndata.AnnData):
    svg = get_svg(adata)
...
def get_svg(adata: anndata.AnnData):
    # SIZING
    width = 1000
    height = 1000

    n_rows_x = adata.n_obs
    n_cols_x = adata.n_vars
    ...
    x_base_obsm = (
        x_base_obsp + size_rows_x + margin_blocks if n_layers_obsp > 0 else x_base_obsp
    )
    x_base = (
        x_base_obsm + size_cols_obsm + margin_blocks
        if n_layers_obsm > 0
        else x_base_obsm
    )
    ...
    y_base_varp = (
        y_base_varm + size_rows_varm + margin_blocks
        if n_layers_varm > 0
        else y_base_varm
    )
    ...
    y_total = (
        y_base_varp + size_cols_x + margin_side + y_extra
        if n_layers_varp > 0
    ...
    x = get_layers(
        ...
        size_cols_x,
        size_rows_x,
        f"({n_rows_x}, {n_cols_x}, {n_layers_x})",
    )
...
def _get_ratio_draw(ratio: float) -> float:
    if ratio > 0.3:
        ratio_draw = ratio
    elif ratio > 0.05:
...
def get_size_x(
    n_rows_x: int, n_cols_x: int, width: int, height: int, ratio_of_canvas: float = 0.3
):
...
def get_layer_table(adata: anndata.AnnData):
    # *layer*    *n* *names*
    # x     1       raw
